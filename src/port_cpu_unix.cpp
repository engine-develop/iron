...
Serial::SerialImpl::waitReadable (uint32_t timeout)
{
  ...
  if (!FD_ISSET (fd_, &readfds)) {
    throw std::runtime_error( "select reports ready to read, but our fd isn't"
  ...
  while (bytes_read < size) {
    ...
    if (waitReadable(timeout)) {
      ...
      if (bytes_read_now < 1) {
        ...
        throw std::runtime_error( "device reports readiness to read but "
      ...
      if (bytes_read > size) {
        throw std::runtime_error( "read over read, too many bytes where "
    ...
    if (r > 0) {
      ...
      if (FD_ISSET (fd_, &writefds)) {
        ...
        if (bytes_written_now < 1) {
          ...
          throw std::runtime_error( "device reports readiness to write but "
                                 "returned no data (device disconnected?)" );
        }
...
{
  if (is_open_ == false) {
    throw std::runtime_error( "Serial::flushOutput" );
  }
...
{
  if (is_open_ == false) {
    throw std::runtime_error( "Serial::sendBreak" );
  }
